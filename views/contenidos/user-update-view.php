<?php
if($loginController->encryption($_SESSION['id_sv']) != $page[1]){
	// COMPROBAR SI EL ID DE LA URL ES EL MISMO DEL USUARIO QUE INICIO SESION Y QUE PRIVILEGIOS TIENE
...
<div class="container-fluid">
	...
	$dataUsu = $insUsu->dataUserController("Unico", $page[1]);

	if($dataUsu->rowCount() == 1){
		$dataArray = $dataUsu->fetch();
	?>
	<form class="form-neon FormularioAjax" autocomplete="off" action="<?php echo SERVERURL; ?>ajax/usuAjax.php"
	method="POST" data-form="update">
	<!-- <form class=" form-neon" action="<?php echo SERVERURL; ?>ajax/usuAjax.php"  method="POST" data-form="update" autocomplete="off"> -->
				...
				<div class="row">
					<div class="col-12 col-md-4">
						<div class="form-group">
							...
							value="<?php echo $dataArray['usuario_dni']; ?> ">
						</div>
					</div>
					...
					<div class="col-12 col-md-4">
						<div class="form-group">
							...
							value="<?php echo $dataArray['usuario_nombre']; ?>">
						</div>
					</div>
					<div class="col-12 col-md-4">
						<div class="form-group">
							...
							value="<?php echo $dataArray['usuario_apellido']; ?>">
						</div>
					</div>
					<div class="col-12 col-md-6">
						<div class="form-group">
							...
							value="<?php echo $dataArray['usuario_telefono']; ?>">
						</div>
					</div>
					<div class="col-12 col-md-6">
						<div class="form-group">
							...
							name="usuario_direccion_up" id="usuario_direccion" maxlength="190"
							value="<?php echo $dataArray['usuario_direccion']; ?>">
						</div>
					</div>
				</div>
			</div>
				...
				<div class="row">
					<div class="col-12 col-md-6">
						<div class="form-group">
							...
							value="<?php echo $dataArray['usuario_usuario']; ?>">
						</div>
					</div>
					<div class="col-12 col-md-6">
						<div class="form-group">
							...
							name="usuario_email_up" id="usuario_email" maxlength="70"
							value="<?php echo $dataArray['usuario_email']; ?>">
						</div>
					</div>
					<?php if($_SESSION['privilegio_sv'] == 1 && $dataArray['usuario_id'] != 1){ ?>
					<div class="col-12">
						<div class="form-group">
							<span>Estado de la cuenta  &nbsp; <?php if($dataArray['usuario_estado'] == "Activa") {
								echo '<span class="badge badge-info">Activa</span>'; }else { echo '<span
									class="badge nadge-danger">Deshabilitada</span>'; } ?></span>
							<select class="form-control" name="usuario_estado_up">
								<option value="Activa" <?php if($dataArray['usuario_estado'] == "Activa"){
								echo 'selected=""'; } ?> >Activa</option>
								<option value="Deshabilitada" <?php if($dataArray['usuario_estado'] == "Deshabilitada"){
					...
					<div class="col-12">
						...
						<div class="form-group">
							<select class="form-control" name="usuario_privilegio_up">
								...
								<option value="1" <?php if($dataArray['usuario_privilegio']==1)
								{ echo 'selected=""';} ?>>Control total - ADMIN <?php
								if($dataArray['usuario_privilegio'] == 1) {echo '(Actual)';}
								?> </option>
								<option value="1" <?php if($dataArray['usuario_privilegio']==2)
								{ echo 'selected=""';} ?>>Edición - VENDENDORES<?php
								if($dataArray['usuario_privilegio'] == 2) {echo '(Actual)';}
								?> </option>
								<option value="1" <?php if($dataArray['usuario_privilegio']==3)
								{ echo 'selected=""';} ?>>Registrar <?php
								if($dataArray['usuario_privilegio'] == 3) {echo '(Actual)';}
								?> </option>
							</select>
						</div>
					</div>
		...
		<?php if($loginController->encryption($_SESSION['id_sv']) != $page[1]) { ?>
